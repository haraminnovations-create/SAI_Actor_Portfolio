...
(function () {
  ...
  function initCoverflow() {
    ...
    /* The wall now carries every photograph in the collection. A carousel
       is not a wall: eighty-seven frames is a long drag, and the section
       calls itself SELECTED FRAMES. The deck therefore takes an even
       stride through the gallery rather than its first two dozen - the
       wall is grouped by category, so the first two dozen would all be
       portraits, and a stride shows the whole range instead. */
    const DECK = 24;
    const step = Math.max(1, Math.ceil(all.length / DECK));
    const src  = all.length > DECK
      ? all.filter((_, k) => k % step === 0).slice(0, DECK)
      : all;
    ...
    const data = src.map((f) => {
      ...
      return {
        ...
        // where this frame sits on the wall, so the viewer opens the right one
        gi:  all.indexOf(f)
      };
    });
    ...
    slides.forEach((s, k) => {
      s.addEventListener('click', () => {
        // the deck is a stride through the wall, so k is not the wall index
        const gi = Number(s.dataset.gi);
        if (window.__ysLightbox) window.__ysLightbox(Number.isFinite(gi) ? gi : k);
        else go(k);
      });
    });
  ...
  function initGallery() {
    ...
    function close() {
      ...
      /* The picture is released only once the viewer is really shut. Open
         another photograph inside the closing animation - easy to do on a
         wall this size - and this timer would otherwise fire late and pull
         the src out from under the frame that had just been asked for,
         which reads to makePlaceholder as a picture that failed to load.
         Removing the attribute rather than blanking it also stops the
         browser resolving '' against the page and reporting an error. */
      setTimeout(() => {
        if (box.classList.contains('open')) return;   // reopened meanwhile
        box.hidden = true;
    ...
    items.forEach((s) => {
      ...
      s.setAttribute('aria-label', 'View photograph' + (s.dataset.cap ? ' — ' + s.dataset.cap : ''));
  ...
  function initAura() {
    ...
    /* the tail's glow is the one part of the pointer that cannot live in the
       ...
       repainted when the theme changes: electric blue over midnight,
       a deeper blue over the paper ground */
    function paintTail() {
      const forest = document.documentElement.getAttribute('data-theme') === 'light';
      // the paper ground takes the deeper blue, the midnight one electric
      const tint = forest ? '37,99,235' : '59,130,246';
      beads.forEach(({ el, i }) => {
  ...
  /* ======================================================================
     11b. THEME — midnight navy by default, paper on request
  ...
  function initTheme() {
    ...
    function apply(theme, remember) {
      ...
      if (meta) meta.setAttribute('content', light ? '#F4F7FA' : '#07111F');
